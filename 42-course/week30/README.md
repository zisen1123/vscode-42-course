# Week 30

Time: 9-11h

Goals:
- Deep training techniques

Tasks:
- regularization, dropout, batchnorm, callbacks
- Keep daily notes in notes/progress.md.
- Add at least 5 test cases in tests/.

Acceptance:
- ablation notes show impact of each technique
- Weekly retrospective includes mistakes and fixes.

Sync tag: 2026-02-15-course-sync

<!-- KNOWLEDGE_MAP_START -->
## Knowledge Mapping
- Hands-On ML (2nd): Ch.11
- Focus: deep training stabilization (dropout, batchnorm, callbacks)
<!-- KNOWLEDGE_MAP_END -->
